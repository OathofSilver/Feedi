...
- 测试计划：`feed_loadtest.jmx`（同目录，threads/rampup/duration 均用 `-J` 传参）
- 结果文件：`fix_50.jtl`、`fix_100.jtl`、HTML 报告 `report_100/`
  —— 原始 JTL 与 HTML 报告体积较大（数十 MB），未随仓库提交；本文档记录结论、修复点与复测方法。
...
```bash
# JMETER_BIN 换成你本机的 apache-jmeter-*/bin 目录
cd "$JMETER_BIN"
./jmeter.bat -n -t feed_loadtest.jmx -Jthreads=100 -Jrampup=10 -Jduration=60 \
  -l fix_100.jtl -e -o report_100
```

前置条件：api 与 worker 均已启动（`../dev.sh start`），MySQL/Redis/RabbitMQ 就绪，测试计划里用到的账号已在库中。
